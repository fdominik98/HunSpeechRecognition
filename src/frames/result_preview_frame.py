...
class ResultPreviewFrame(CTkFrame):
    def __init__(self, parent, row, column, settings: Settings, result_manager : ResultManager, split_audio_file_manager : SplitAudioFileManager, trimmed_audio_file_manager: TrimmedAudioFileManager, audio_load_callback, audio_play_callback):
        ...
        self.arrow_up_icon = CTkImage(Image.open(f'{get_images_path()}/arrow_up.png'), size=(17, 17))
        self.arrow_down_icon = CTkImage(Image.open(f'{get_images_path()}/arrow_down.png'), size=(17, 17))
        ...
        self.search_result_label.grid(row=0, padx=205, pady=(10,5), sticky="se")

        self.search_by_value_entry_val = StringVar(value="0")
        self.search_by_value_entry_val.trace_add("write", self.__go_to_searched_text)
        self.search_by_value_entry = CTkEntry(self, width=50, height=10, font=label_font(), textvariable=self.search_by_value_entry_val)
        self.search_by_value_entry.grid(row=0, padx=150, pady=(10,0), sticky="se")

        self.down_button = CTkButton(self, fg_color="transparent", text="", corner_radius=100, width=5, height=5, image=self.arrow_down_icon
                                                    ,command=self.__go_to_next)
        self.down_button.grid(row=0, padx=120, pady=(25,0), sticky="se")

        self.up_button = CTkButton(self, fg_color="transparent", text="", corner_radius=100, width=5, height=5, image=self.arrow_up_icon
                                                    ,command=self.__go_to_previous)
    ...
    def __go_to_searched_text(self, *args):
        if self.__search_results != []:
            self.textbox.tag_remove("selected_search_tag", "1.0", END)
            new_value = int(self.search_by_value_entry_val.get()) - 1 if self.search_by_value_entry_val.get().isdecimal() else -1
            if new_value >= 0 or new_value < len(self.__search_results):
                new_range = self.__search_results[new_value]
                self.textbox.tag_add("selected_search_tag", new_range[0], new_range[1])
